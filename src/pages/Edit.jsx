...
const Edit = () => {
  ...
  const { values, handleSubmit, handleChange, setFieldValue, errors, touched } =
    useFormik({
      ...
      onSubmit: (values) => {
        console.log(values)
        data[state.id] = { name: values.name, country: values.country, websiteUrl: values.websiteUrl };
  ...
  useEffect(() => {
    // setFieldValue("name", "df")
      setFieldValue("name", data[state.id]?.name);
      setFieldValue("country", data[state.id]?.country);
      setFieldValue(
        "websiteUrl",
        data[state.id]?.websiteUrl ? data[state.id]?.websiteUrl : data[state.id]?.domains[0]
      );
  }, []);
  ...
  return (
    ...
    >
      <h3 className="flex justify-center text-slate-500">Edit</h3>
      <div className="my-2 w-full">
        ...
        {errors.name && touched.name ? (
          <div className="text-xs text-red-700 mx-1">Name is redquired</div>
        ) : null}
      </div>
      <div className="mb-2 w-full ">
        ...
        {errors.websiteUrl && touched.websiteUrl ? (
          <div className="text-xs text-red-700 mx-1">
            Website url is redquired
          </div>
        ) : null}
      </div>
      <div className="mb-2 w-full">
        ...
        {errors.country && touched.country ? (
          <div className="text-xs text-red-700 mx-1">country is redquired</div>
        ) : null}
      </div>
